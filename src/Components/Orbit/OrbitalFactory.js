...
function make_orbit_keyframes(t_max, orbit_radius, phaseShift, tiltZ) {
    ...
    function _single_orbit_keyframe(t, t_max, R, phaseShift, tiltZ){
        const
            ...
            zIndex = 2 + Math.floor(Math.cos(angle_rad+Math.PI)),
...
export const Orbital = (diameter=40, orbit_radius=350, phaseShift=0, tiltZ=0, initial_velocity=15) => {
    const DEFAULT_ORBIT_RADIUS = 300; // proportionality constant used to simulate kepler's law, distinct from default value of `orbit_radius` arg
    const N_FRAMES = 100;
    ...
    const period = Math.floor((35+Math.random()*initial_velocity) * ((orbit_radius**2)/(Math.sqrt(diameter/10)*DEFAULT_ORBIT_RADIUS**2)) )

    return styled.div`
        z-index: 0;
        transform: perspective(100px) translateX(${Math.floor(orbit_radius * Math.sin(phaseShift))}) scaleX(${scale}) scaleY(${scale});
        position: absolute;
        text-align: center;
        color: white;
        width: ${diameter}px;
